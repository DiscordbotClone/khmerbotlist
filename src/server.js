  ...
  const renderTemplate = (res, req, template, data = {}) => {
   ...
   app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://khmerbotlist.xyz/user/${a.id}) Login To Website!`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "You have been blocked from khmerbotlist.", logout: true })
        req.logout();
        });
        } else {
            try {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://khmerbotlist.xyz/user/${a.id}) Login To Website!`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/dc", (req, res) => {
      res.redirect('https://discord.gg/8cNQb9rDue');
    });  
    ...
    app.get("/robots.txt", function(req,res) {
      ...
      res.send(`Sitemap: https://khmerbotlist.xyz/sitemap.xml`);
    });

    app.get("/sitemap.xml", async function(req,res) {
        let link = "<url><loc>https://khmerbotlist.xyz/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://khmerbotlist.xyz/bot/"+bot.botID+"</loc></url>";
        })
    ...
    app.get("/bot/:botID/delete", async (req, res) => {
      ...
      app.post("/admin/decline/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
       ...
       app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`The user named **[${req.user.username}](https://khmerbotlist.xyz/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
          .addField("Code Link", `https://khmerbotlist.xyz/code/${kod}`, true)
       ...
       app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
          ...
          .setDescription(`The user named **[${req.user.username}](https://khmerbotlist.xyz/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
          .addField("Code Link", `https://khmerbotlist.xyz/code/${kod}`, true)
          .addField("Code Description", rBody['codedesc'], true)
          .addField("Code Category", rBody['category'], true)
          )
      ...
      app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`<:agent_online:844329716382433300> khmerbotlist has been switched to __maintance__ due to **${req.body.reason}**`).then(a => { 
      ...
      app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        .setAuthor("khmerbotlist.xyz", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<:agent_online:844329716382433300> khmerbotlist are **active** again!\n[Click to redirect website](https://khmerbotlist.xyz)`)
        .setFooter("khmerbotlist © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => { a.edit(`~~ <:agent_online:844329716382433300> khmerbotlist has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesvcodes) } )
   ...
   //------------------- API  -------------------//
    app.get("/api", async (req, res) => {
      res.json({ "Hello": "World", "Host by": "khmerbotlist.xyz"});
    });
